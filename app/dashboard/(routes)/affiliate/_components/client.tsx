...
export const AffiliateClient = () => {
  ...
  const { data: history, isSuccess: historyIsSuccess } = useGetAffiliateHistories({
    page: currentPage,
  });
  ...
  useEffect(() => {
    if (historyIsSuccess) {
      ...
      if (histories) {
        ...
        const historiesFormatted: IAffiliateHistory[] = histories.data.map((item) => ({
          id: item.id,
  ...
  return (
    <>
      <Heading title={`Welcome back ${user.first_name}`} description="Total Earning is: 0,00$" drawEarning />
      <AffiliateCode code={user.referred_code} />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <StatisticCard color="#26a6a4" amount={statistics?.data.this_month_earnings ?? 0} label="This Month" />
        <StatisticCard color="#f63d68" amount={statistics?.data.this_year_earnings ?? 0} label="This Year" />
        <StatisticCard color="#7a5af8" amount={statistics?.data.total_earnings ?? 0} label="All Time" />
      </div>
      <DataTable
        columns={columns}
        data={data}
