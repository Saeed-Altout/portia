...
import { BlogItemCard } from "@/components/root/cards/blog-item-card";
      ...
      <Container className="gap-y-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="basis-1/2">
            {blogsData.map((blog, index) => (
              <React.Fragment key={index}>
                {blog.isFavorite && blog.order === 1 && <BlogCard initialData={blog} />}
              </React.Fragment>
            ))}
          </div>
          <div className="basis-1/2 flex flex-col gap-8">
            {blogsData.map((blog, index) => (
              <React.Fragment key={index}>
                {blog.isFavorite && blog.order !== 1 && <BlogItemCard initialData={blog} />}
              </React.Fragment>
            ))}
          </div>
        </div>
