...
export const Content = () => {
  ...
  useEffect(() => {
    if (isSuccess) {
      const features =
        offers?.data?.[0]?.package?.feature_groups?.[0]?.features?.map(
          (feature) => ({
            name: feature.name,
        ...
        {(isLoading || !isSuccess) && (
          <div className="w-full grid grid-cols-1 md:grid-cols-4">
            {[...Array(3)].map((_, offerIndex) => (
              <div
                key={offerIndex}
                className={cn(offerIndex === 0 && "col-span-1 md:col-span-2")}
              >
                <div
                  className={cn(
                  ...
                  <ul key={groupIndex} className="flex flex-col border-b">
                    ...
                    {[...Array(3)].map((_, featureIndex) => (
                      ...
                      >
                        <Skeleton
                          className={cn(
                            "h-5 w-20 rounded-md mr-auto",
                            offerIndex !== 0 && "md:hidden"
                          )}
                        />
                        <span
                          className={cn(
                            offerIndex === 0 && "md:w-1/2 md:ml-auto"
                          )}
                        >
              ...
              <div className="flex flex-col" key={index}>
                ...
                <div className="h-[350px]">
                  ...
                  <div className="flex-1 flex flex-col justify-between items-start gap-y-6 py-6 px-4">
                    ...
                    <p className="text-sm flex-1 min-h-[40px] line-clamp-2">
                ...
                <div className="flex-1">
                  <div className="h-full">
                    {offer.package.feature_groups.map(
                      (feature_group, index) => (
